...
void PriorityQueue::PopNextBatch(int batch_size)
{
    ...
    // 每个线程本地收集猜测，最后合并
    std::vector<std::vector<std::string>> thread_guesses(actual_batch);

    // 并行处理每个PT
    #pragma omp parallel for
    for (int i = 0; i < actual_batch; ++i) {
        // 这里直接调用GenerateBatch，输出到thread_guesses[i]
    ...
    for (int i = 0; i < actual_batch; ++i) {
        ...
        for (PT &pt : new_pts) {
            ...
            while (iter != priority.end() && pt.prob < iter->prob) {
                ++iter;
            }

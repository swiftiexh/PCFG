...
void MD5Hash_SIMD(array<std::string,8> input, bit32 state[8][4])
{
    ...
    uint32x8_t a = {
        vdupq_n_u32(0x67452301),   // 前四个
        vdupq_n_u32(0x67452301)    // 后四个
    };
    uint32x8_t b = {
	...
	for (int i = 0; i < n_blocks; i += 1)
	{
        ...
        for (int i1 = 0; i1 < 16; i1++) 
		{
		  ...
		  {
			//想加载32位
			vals[i2] = (paddedMessages[i2][4 * i1 + i * 64])|
			(paddedMessages[i2][4 * i1 + 1 +i * 64] << 8)|
			(paddedMessages[i2][4 * i1 + 2 + i * 64] << 16)|
			(paddedMessages[i2][4 * i1 + 3 + i * 64] << 24);
          }
          x[i1].val[0] = vld1q_u32(&vals[0]);  // 前 4 路
          x[i1].val[1] = vld1q_u32(&vals[4]);  // 后 4 路
        }


        uint32x8_t original_a = a;
        uint32x8_t original_b = b;
        uint32x8_t original_c = c;
        uint32x8_t original_d = d;
		...
		uint32_t tempa[8], tempb[8], tempc[8], tempd[8];
        vst1q_u32(&tempa[0], a.val[0]);
        vst1q_u32(&tempa[4], a.val[1]);
        vst1q_u32(&tempb[0], b.val[0]);
        vst1q_u32(&tempb[4], b.val[1]);
        vst1q_u32(&tempc[0], c.val[0]);
        vst1q_u32(&tempc[4], c.val[1]);
        vst1q_u32(&tempd[0], d.val[0]);
        vst1q_u32(&tempd[4], d.val[1]);
		for (int i = 0; i < 8; i++) {
			state[i][0] += tempa[i];  
			state[i][1] += tempb[i]; 
			state[i][2] += tempc[i]; 
        ...
        a.val[0] = vaddq_u32(a.val[0], original_a.val[0]);
        a.val[1] = vaddq_u32(a.val[1], original_a.val[1]);
        b.val[0] = vaddq_u32(b.val[0], original_b.val[0]);
        b.val[1] = vaddq_u32(b.val[1], original_b.val[1]);
        c.val[0] = vaddq_u32(c.val[0], original_c.val[0]);
        c.val[1] = vaddq_u32(c.val[1], original_c.val[1]);
        d.val[0] = vaddq_u32(d.val[0], original_d.val[0]);
        d.val[1] = vaddq_u32(d.val[1], original_d.val[1]);
	}
	...
	for(int i1 = 0; i1 < 8; i1++)
	{
	  for (int i = 0; i < 4; i++)
	  {
		  uint32_t value = state[i1][i];
		  state[i1][i] = ((value & 0xff) << 24) |		 // 将最低字节移到最高位
